...
@NamedNativeQuery(
        ...
        query = "SELECT p.id, p.name, p.sale_price as price, p.product_view as views, p.slug, p.total_sold, replace(JSON_EXTRACT(p.images, '$[0]'), '\"', '')  AS images " +
...
@NamedNativeQuery(
        ...
        query = "SELECT DISTINCT d.* " +
                "FROM (" +
                "SELECT DISTINCT product.id, product.name, product.slug, product.sale_price as price, product.product_view as views, product.total_sold, replace(JSON_EXTRACT(product.images, '$[0]'), '\"', '')  AS images " +
                "FROM product " +
...
@NamedNativeQuery(
        ...
        resultSetMapping = "productInfoDto",
        query = "SELECT DISTINCT product.id, product.name, product.slug, product.sale_price as price, product.product_view as views, product.total_sold, replace(JSON_EXTRACT(product.images, '$[0]'), '\"', '')  AS images " +
                "FROM product " +
                "INNER JOIN product_category " +
                "ON product.id = product_category.product_id " +
                "WHERE product.status = 1 AND product.brand_id IN (?1) AND product_category.category_id IN (?2) " +
...
@NamedNativeQuery(
        ...
        resultSetMapping = "productInfoDto",
        query = "SELECT DISTINCT p.id, p.name, p.slug, p.sale_price as price, p.product_view as views, p.total_sold, replace(JSON_EXTRACT(p.images, '$[0]'), '\"', '') AS images " +
                "FROM product p " +
