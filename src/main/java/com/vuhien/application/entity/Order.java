...
@NamedNativeQuery(
        ...
        query = "SELECT od.id, od.total_price, od.size size_vn, p.name product_name, replace(JSON_EXTRACT(p.images, '$[0]'), '\"', '') as product_img " +
                "FROM orders od " +
...
@NamedNativeQuery(
        ...
        query = "SELECT orders.id, orders.total_price, orders.size size_vn, product.name product_name, orders.price as product_price, " +
                ...
                "replace(JSON_EXTRACT(product.images, '$[0]'), '\"', '') as product_img " +
                "FROM orders " +
                "INNER JOIN product " +
